...
The default help fetches `/api/client` once and renders a compact, state-aware overview: the product summary, a one-line auth status, the server-recommended next steps, and a grouped command summary, with pointers to the fuller views. The catalog is derived from the selected bearer session, so the same request returns different guidance before and after login; the CLI only lays out what the Worker sends. Two server-owned fields drive the state-specific parts — `auth` (status `anonymous`/`user`/`agent`, identity, and for delegated agents an agent block) and `nextSteps` (an ordered `{title, command, description}` list whose commands are ready to run, with the known workspace id pre-substituted for agents). Both are additive (schema version unchanged); when an older server omits them the CLI falls back to a minimal hint and skips the status line. `craken help --verbose` renders the full reference (every server help section, the local bootstrap commands and flags, every command with descriptions and examples, and every operation route); `craken help --format json` emits the `auth`/`nextSteps` guidance as JSON for coding agents. Operation help such as `craken channel messages --help` focuses on the matching server command when it is present, then augments it with local option knowledge and any route metadata the catalog exposes. Authentication mechanics and generic transport flags stay local bootstrap behavior; product explanation, the recommended next steps, and the product command surface belong to the Worker so API changes do not require a CLI release just to discover or execute new operations.
